...
4. Implement engaging, UX-first, performant game systems
...
When working on game code, activate these skills proactively:
- **game-perf** - For per-frame hot-path code (CRITICAL for JS/TS games)
- **typescript-pro** - For TypeScript projects

## JavaScript/TypeScript Hot-Path Rules

For browser/Node.js games, per-frame code must avoid GC pressure:

**Never in hot paths:**
- Spread operators: `[...array]` creates new array
- `array.filter()` / `array.map()` - always allocate
- Object literals `{}` or array literals `[]` in loops
- String concatenation for spatial grid keys

**Always use instead:**
- Readonly refs: `array as readonly T[]`
- In-place filtering with swap-and-truncate
- Module-level scratch arrays with `readonly` return types
- Spatial hash grids for O(n²) → O(n) proximity queries
- Reused context objects for per-entity operations

See the **game-perf** skill for patterns and code templates.
