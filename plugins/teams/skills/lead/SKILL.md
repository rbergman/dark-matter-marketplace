...
| `bd backup export-git` | Lead (session end) |

**Sync flow:** `bd ready` at session start to find work, create team tasks from bead items, `bd close` after team delivers, `bd backup export-git` before cleanup.
...
- [ ] `bd backup export-git` to sync beads state
- [ ] All work committed
- [ ] Team cleaned up
