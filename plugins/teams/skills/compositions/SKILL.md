...
**Pattern:** beads = persistent cross-session state, Agent Teams tasks = session-level coordination. Beads survive session boundaries; team tasks do not. Always sync beads state (`bd export`) before the session ends.
