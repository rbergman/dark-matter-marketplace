...
**Pattern:** beads = persistent cross-session state, Agent Teams tasks = session-level coordination. Beads survive session boundaries; team tasks do not. Always push results to remote (`bd dolt commit && bd dolt push`) before the session ends.
...
6. `bd dolt commit && bd dolt push` to persist state
