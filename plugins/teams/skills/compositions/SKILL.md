...
**Pattern:** beads = persistent cross-session state, Agent Teams tasks = session-level coordination. Beads survive session boundaries; team tasks do not. Always sync beads state (`bd export -o .beads/issues.jsonl`) before the session ends.
...
6. `bd export -o .beads/issues.jsonl` to persist state
